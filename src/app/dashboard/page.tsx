...
type SearchParams = Promise<{ onboarded?: string; invited?: string; error?: string; approval?: string }>;

export default async function DashboardPage({ searchParams }: { searchParams: SearchParams }) {
  const params = await searchParams;
  const { supabase, user, organizationId, locationId, role, onboardingStatus, isApproved, memberships, locations } = await getDashboardContext();
  ...
  const [businessResult, locationsResult] = await Promise.all([
    supabase.from("organizations").select("name, category, city, plan, public_status, onboarding_status, rejection_reason, logo_emoji").eq("id", organizationId).single(),
    supabase.from("locations").select("id", { count: "exact", head: true }).eq("organization_id", organizationId).eq("is_active", true),
  ]);
  const business = businessResult.data;
  const [cardsResult, offersResult, stampsResult, subscriptionResult] = isApproved ? await Promise.all([
  ...
  const roleLabel = role === "owner" ? "Inhaber" : role === "manager" ? "Manager" : "Mitarbeiter";
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,#0b4f63_0%,#061827_35%,#020617_100%)] px-5 py-14 text-white sm:px-8"><section className="mx-auto max-w-7xl">
    <div className="flex flex-wrap items-start justify-between gap-6"><div><span className="inline-flex rounded-full border border-cyan-300/35 bg-cyan-300/10 px-4 py-2 text-sm font-black text-cyan-300">Geschäfts-Dashboard · Rolle: {roleLabel}</span><h1 className="mt-5 text-5xl font-black">{business?.logo_emoji ?? "🏪"} {business?.name ?? "TapRadar"}</h1><p className="mt-3 text-slate-300">{business?.category}{business?.city ? ` · ${business.city}` : ""}</p></div><form action="/logout" method="post"><button type="submit" className="rounded-2xl bg-white/10 px-4 py-3 font-black">Abmelden</button></form></div>
    ...
    {!isApproved ? <div className="mt-8 rounded-3xl border border-amber-300/30 bg-amber-300/10 p-6 text-amber-50"><p className="text-xs font-black uppercase tracking-[.18em]">Freigabe ausstehend</p><h2 className="mt-3 text-2xl font-black">Dein Geschäft wird von TapRadar geprüft.</h2><p className="mt-3 max-w-3xl leading-7 text-amber-100/85">Bis zur Freigabe kannst du Unternehmensdaten und Filialen vervollständigen. Alle weiteren Portalbereiche werden nach der Prüfung automatisch freigeschaltet.</p>{business?.rejection_reason ? <p className="mt-4 rounded-2xl bg-red-300/10 p-4 text-red-100">Rückfrage der Administration: {business.rejection_reason}</p> : null}<p className="mt-4 text-sm">Status: <strong>{onboardingStatus ?? business?.onboarding_status ?? "in Prüfung"}</strong></p></div> : null}
    <div className={`mt-8 grid gap-4 sm:grid-cols-2 ${isApproved ? "lg:grid-cols-5" : "lg:grid-cols-2"}`}><Metric label="Filialen" value={locationsResult.count ?? 0} />{isApproved ? <><Metric label="Treuekarten" value={cardsResult.count ?? 0} /><Metric label="Aktive Angebote" value={offersResult.count ?? 0} /><Metric label="Stempel gesamt" value={stampsResult.count ?? 0} /><Metric label="Tarif" value={`${subscriptionResult.data?.plan ?? business?.plan ?? "bronze"}`} detail={subscriptionResult.data?.status ?? "incomplete"} /></> : <Metric label="Prüfstatus" value="Ausstehend" detail="Freigabe durch Administration" />}</div>
    ...
    <h2 className="mt-10 text-3xl font-black">Verwaltung</h2><div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3"><Nav href="/dashboard/business" title="Geschäftsdaten" text="Firma, Rechnungsadresse, UID und Beschreibung." /><Nav href="/dashboard/locations" title="Filialen" text="Standorte, Öffnungszeiten und Sichtbarkeit." />{isApproved ? <><Nav href="/dashboard/loyalty-cards" title="Treuekarten & Belohnungen" text="Leistung, Stempelziel und verdiente Belohnung." /><Nav href="/dashboard/actions" title="Aktionen" text="Kampagnen, Neuigkeiten, Bilder und PDFs." /><Nav href="/dashboard/vouchers" title="Gutscheine" text="Gutscheinwerte, Kassencodes und Live-Vorschau." /><Nav href="/dashboard/devices" title="QR / NFC Geräte" text="Sichere, widerrufbare Stempel-Zugänge." /><Nav href="/dashboard/redeem" title="Belohnung einlösen" text="Kundencode an der Kasse bestätigen." /><Nav href="/dashboard/analytics" title="Filialstatistik" text="Kennzahlen für die aktuell gewählte Filiale." /><Nav href="/dashboard/privacy" title="Datenschutz und Konto" text="Datenexport und Kontolöschung verwalten." />{canManage ? <><Nav href="/dashboard/team" title="Team und Rechte" text="Mitarbeiter einladen und verwalten." /><Nav href="/dashboard/media" title="Medien und Dateien" text="Bilder, Logos und PDF-Werbung." /><Nav href="/dashboard/billing" title="Tarif und Rechnungen" text="Rechnungsarchiv, Steuer und Zahlungsmittel." /></> : null}</> : null}</div>
    <div className="mt-10 flex flex-wrap gap-5">{isApproved ? <Link href="/dashboard/onboarding?new=1" className="font-black text-cyan-300">Weiteres Unternehmen anlegen</Link> : null}<Link href="/" className="font-black text-cyan-300">Zur Website</Link>{isApproved ? <a href="/api/v1/sync" className="font-black text-cyan-300">App-Sync API prüfen</a> : null}</div>
  </section></main>;
}
