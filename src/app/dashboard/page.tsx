...
export default async function DashboardPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,#0b4f63_0%,#061827_35%,#020617_100%)] px-5 py-14 text-white sm:px-8"><section className="mx-auto max-w-7xl">
    <div className="flex flex-wrap items-start justify-between gap-6"><div><span className="inline-flex rounded-full border border-cyan-300/35 bg-cyan-300/10 px-4 py-2 text-sm font-black text-cyan-300">Geschäfts-Dashboard · {role}</span><h1 className="mt-5 text-5xl font-black">{business?.logo_emoji ?? "🏪"} {business?.name ?? "TapRadar"}</h1><p className="mt-3 text-slate-300">{business?.category}{business?.city ? ` · ${business.city}` : ""}</p></div><Link href="/logout" className="rounded-2xl bg-white/10 px-4 py-3 font-black">Abmelden</Link></div>
