      ...
      <div className="mt-5 grid content-start gap-4">
        {offers.map((offer) => {
          ...
          return <details key={offer.id} className="group rounded-[24px] border border-white/10 bg-white/[0.07]">
            ...
            <OfferForm offer={offer} locations={locations} mediaUrl={mediaUrl} mediaType={asset?.mime_type ?? null} fixedType={type} showVoucherPreview={isVoucher} previewBusinessName={organization?.name ?? "Firmenname"} className="border-t border-white/10 p-5" />
          </details>;
        })}
