...
export async function savePaymentProvider(formData: FormData) {
  const { supabase } = await requirePlatformAdmin(); const provider = requiredText(formData.get("provider"), 20); const mode = requiredText(formData.get("mode"), 10); if (!["stripe", "manual"].includes(provider) || !["test", "live"].includes(mode)) redirect("/admin/payments?error=invalid");
