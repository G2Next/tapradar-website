...
export type InvoiceSettingsState = { error?: string };

export async function updateInvoiceSettings(_prevState: InvoiceSettingsState, formData: FormData): Promise<InvoiceSettingsState> {
  ...
  if (!/^#[0-9a-f]{6}$/i.test(color) || !["left", "center", "right"].includes(position) || !Number.isInteger(days) || days < 0 || days > 365) return { error: "settings" };
  ...
  if (!payload.company_name || !/^[A-Z]{2}$/.test(String(payload.country_code))) return { error: "settings" };
  const file = formData.get("logo"); if (file instanceof File && file.size > 0) { if (file.size > 2_097_152 || !["image/png", "image/jpeg"].includes(file.type)) return { error: "logo" }; const extension = file.type === "image/png" ? "png" : "jpg"; const path = `invoice/logo-${Date.now()}.${extension}`; const { error: uploadError } = await supabase.storage.from("platform-assets").upload(path, file, { contentType: file.type, upsert: false }); if (uploadError) return { error: "logo-upload" }; const { data: publicUrl } = supabase.storage.from("platform-assets").getPublicUrl(path); payload.logo_path = path; payload.logo_url = publicUrl.publicUrl; }
  const { error } = await supabase.from("invoice_settings").update(payload).eq("id", true); if (error) return { error: "settings-save" }; revalidatePath("/admin/billing"); revalidatePath("/dashboard/billing"); redirect("/admin/billing?saved=settings");
}
