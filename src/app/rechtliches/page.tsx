    ...
    <section className="mx-auto max-w-2xl rounded-[32px] border border-white/10 bg-white/[0.07] p-6 shadow-2xl shadow-black/25 sm:p-9">
      ...
      <form action={acceptAccountLegal} className="mt-7 grid gap-4">
        ...
        <label className="flex items-start gap-3 rounded-2xl border border-white/10 bg-slate-950/30 p-4 leading-6">
          ...
          <span>Ich akzeptiere die <Link href={isBusiness ? "/agb-geschaeftskunden" : "/agb-verbraucher"} target="_blank" rel="noopener noreferrer" className="font-black text-cyan-300 underline">{isBusiness ? "Geschäftskunden-AGB" : "Verbraucher-AGB"}</Link> (Version {isBusiness ? LEGAL_VERSIONS.businessTerms : LEGAL_VERSIONS.terms}).</span>
        </label>
        <label className="flex items-start gap-3 rounded-2xl border border-white/10 bg-slate-950/30 p-4 leading-6">
        ...
        {isBusiness ? null : <p className="text-sm leading-6 text-slate-400">Informationen für Verbraucher findest du zusätzlich in der <Link href="/widerrufsbelehrung" target="_blank" rel="noopener noreferrer" className="font-bold text-cyan-300 underline">Widerrufsbelehrung</Link>. Eine Zustimmung zu optionaler Werbung ist hiervon getrennt.</p>}
