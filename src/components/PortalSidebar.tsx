...
export function PortalSidebar({ mode }: { mode: "admin" | "dashboard" }) {
  ...
  return <>
    <aside className="sticky top-[73px] hidden h-[calc(100vh-73px)] w-64 shrink-0 overflow-y-auto border-r border-white/10 bg-[#07111f] p-5 lg:block">
      <p className="mb-5 px-3 text-xs font-black uppercase tracking-[0.18em] text-slate-500">{title}</p>{links}
    </aside>
    <details className="border-b border-white/10 bg-[#07111f] px-5 py-3 text-white lg:hidden">
      <summary className="cursor-pointer font-black">Menü · {title}</summary><div className="mt-3">{links}</div>
    </details>
  </>;
}
